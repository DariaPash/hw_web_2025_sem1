...
def question_view(request, question_id):
    ...
    if request.user.is_authenticated:
        ...
        # Получаем лайки пользователя для всех ответов на странице
    ...
    if request.method == 'POST':
        ...
        if answer_form.is_valid():
            ...
            # Находим страницу, на которой находится новый ответ
            # Поскольку ответы отсортированы по -is_correct, -created_at,
            # новый ответ будет в начале списка (если не помечен как правильный)
            # или в начале правильных ответов (если помечен)
...
def ask(request):
    # Если пользователь не авторизован, редиректим на логин с continue
    if not request.user.is_authenticated:
        return redirect(f"{reverse('login')}?continue={request.path}")
...
def search_suggestions(request):
    """AJAX endpoint для поисковых подсказок"""
    query = request.GET.get('q', '').strip()
...
def login_view(request):
    ...
    if request.method == 'POST':
        ...
        # Получаем next из POST или используем сохраненный из GET
        ...
        if form.is_valid():
            ...
            if user is not None:
                ...
                if next_url:
                    # Проверяем, что это безопасный редирект (не внешний URL)
                    if next_url.startswith('/'):
                        return redirect(next_url)
                    # Если это именованный URL, пробуем использовать его
                    try:
                        from django.urls import reverse
                        return redirect(next_url)
                    except:
...
def profile_edit(request):
    ...
    if request.method == 'POST':
        ...
        # Убеждаемся, что initial значения установлены
...
def profile(request, username):
    ...
    # Обработка формы настроек (только для владельца профиля)
    if request.user.is_authenticated and request.user == profile_user and request.method == 'POST':
        ...
        form = ProfileEditForm(request.POST, request.FILES, instance=profile_obj, user=profile_user)
        if form.is_valid():
            form.save()
            messages.success(request, 'Профиль успешно обновлен!')
        ...
        if request.user.is_authenticated and request.user == profile_user:
            ...
            if not form.initial.get('first_name') and profile_user.first_name:
                form.initial['first_name'] = profile_user.first_name
            if not form.initial.get('email') and profile_user.email:
                form.initial['email'] = profile_user.email
...
def question_like(request, question_id):
    """AJAX endpoint для лайка/дизлайка вопроса"""
    if not request.user.is_authenticated:
        return JsonResponse({'error': 'Требуется авторизация'}, status=401)
    
    question = get_object_or_404(Question, id=question_id)
    like_type = request.POST.get('type', 'like')  # 'like' или 'dislike'
    is_positive = like_type == 'like'
    
    # Проверяем, есть ли уже лайк от этого пользователя
    try:
        like = QuestionLike.objects.get(question=question, user=request.user)
        # Если тип лайка тот же, удаляем его (отмена)
...
def answer_like(request, answer_id):
    """AJAX endpoint для лайка/дизлайка ответа"""
    if not request.user.is_authenticated:
        return JsonResponse({'error': 'Требуется авторизация'}, status=401)
...
def mark_answer_correct(request, question_id, answer_id):
    """AJAX endpoint для отметки правильного ответа"""
    if not request.user.is_authenticated:
        return JsonResponse({'error': 'Требуется авторизация'}, status=401)
    ...
    # Переключаем статус правильного ответа
